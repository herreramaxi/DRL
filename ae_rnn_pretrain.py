...
def collect_sequences():
    ...
    seqs = []
    pbar = tqdm(total=N_SAMPLES//SEQ_LEN, desc="Collecting sequences")
    while len(seqs) < N_SAMPLES // SEQ_LEN:
        buf = []
        obs, _ = env.reset()
        done = False
    ...
    np.save(SAVE_BOARDS, arr)
    print(f"Saved {arr.shape[0]} sequences → {SAVE_BOARDS}")
